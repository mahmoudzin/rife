...
interface IGoalProps {
  goal: any;
  disabled: boolean;
  setDisabled: (flag: boolean | ((prevState: boolean) => boolean)) => void;
}

function GoalHeader({ goal, disabled, setDisabled }: IGoalProps) {
  return (
...
interface IGoalBodyProps extends IGoalProps {
  setGoal: (state: { new: number; review: number }) => void;
}
function GoalBody({ goal, setGoal, disabled, setDisabled }: IGoalBodyProps) {
  return (
    <div className="mt-4">
      <p className="mb-2">
        ☘️ New Word:
        <input
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            setGoal({
              ...
              review: isNaN(parseInt(e.target.value))
                ? 0
