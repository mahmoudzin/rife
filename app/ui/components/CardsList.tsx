...
export default function LastCards({ words }: { words: any }) {
  const [isVisible, setIsVisible] = useState<{ show: boolean; item: any }>({
    show: false,
    item: null,
  });
  ...
  const getId = useCallback((id: number) => {
    setIsVisible({
      show: true,
      item: words.find((word: any) => word.wordID === id),
    });
  }, []);
